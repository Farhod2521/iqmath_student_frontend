...
const subscriptionAPI = {
  ...
  create: async (payload) => {
    const { data } = await request.post('/api/v1/payments/superadmin/subscription-benefits/', payload)
    return data
  },
  update: async ({ id, ...payload }) => {
    const { data } = await request.put(`/api/v1/payments/superadmin/subscription-benefits/${id}/`, payload)
    return data
  },
  delete: async (id) => {
    await request.delete(`/api/v1/payments/superadmin/subscription-benefits/${id}/`)
  }
}
...
export default function Benefits() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['subscription-benefits'])
      resetForm()
    }
  })
            ...
            <tbody>
              {benefits.map((item) => (
                <tr key={item.id} className="border-t">
                  ...
                  <td className="p-2 sm:p-3">{item.description_uz || '---'}</td>
                  <td className="p-2 sm:p-3">{item.description_ru || '---'}</td>
