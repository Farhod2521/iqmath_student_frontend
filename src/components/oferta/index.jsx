...
export default function UserAgreement() {
  ...
  // PDF manzilini olish
  const pdfUrl = i18n.language === 'uz' ? get(ofertas, 'data[0].pdf_uz') : get(ofertas, 'data[0].pdf_ru')
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      />
      <label className="cursor-pointer text-sm my-[10px]" onClick={() => setIsModalOpen(true)}>
        {t('oferta')}
      </label>
        ...
        <div className="fixed inset-0 flex items-center w-full justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg max-w-2xl shadow-lg w-[1200px]">
            ...
            <div className="mt-4 flex justify-end space-x-2">
              ...
              <button onClick={handleAgree} className={`px-4 py-2 bg-blue-500 text-white rounded `}>
                {t('submit')}
              </button>
            </div>
          </div>
