...
const InputPhone = React.forwardRef(({ className = '', ...props }, ref) => {
  return (
    <div className="flex items-center border rounded-[8px] px-3 py-2.5" style={{background:'rgba(255,255,255,0.4)',borderColor:'rgba(255,255,255,0.7)'}}>
      ...
      <div className="w-px h-5" style={{background:'rgba(255,255,255,0.7)'}} />
      <input
        ref={ref}
        className={`w-full text-sm bg-transparent text-white placeholder-[#e5e5e5] border-none ml-2 focus:outline-none ${className}`}
        style={{border:'none'}}
        type="tel"
        maxLength={9}
        inputMode="numeric"
