...
import { useGetQuery } from "@/hooks/useGetQuery"; // sening hook
...
const TopicDropdown = ({ chapterId, session }) => {
  ...
  return (
    <div className="relative flex items-center w-full overflow-hidden">
      <button
        className="absolute left-0 z-10 bg-white text-xl px-2 py-1"
      ...
      <div
        ref={scrollRef}
        className="flex overflow-x-auto gap-2 px-8 py-2 scroll-smooth no-scrollbar"
      >
        {topics?.map((topic) => (
          <button
            key={topic.id}
      ...
      <button
        className="absolute right-0 z-10 bg-white text-xl px-2 py-1"
