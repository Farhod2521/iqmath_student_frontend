  ...
  const handleDragEnd = (event) => {
    ...
    if (active.id !== over.id) {
      setSubjects((items) => {
        ...
        // API ga yangi tartibni yuborish
        reorderSubjects(
          { subjects: newItems.map((item, index) => ({ id: item.id, order: index + 1 })) },
          {
            onSuccess: () => {
              toast.success(t("subjectsReordered"));
            },
            onError: () => {
              toast.error(t("errorReorderingSubjects"));
            },
          }
        );
